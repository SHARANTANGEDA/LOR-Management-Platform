...
export const convertGoogleToken = (access_token, imageUrl) => async dispatch => {
	...
	axios.post('/auth/convert-token/', searchParams).then(res => {
		axios.post('/api/auth/getRole', searchParams).then(roleRes => {
			console.log({IMG: localStorage.getItem("google_avatar_url"), item: imageUrl});
			axios.post('/api/auth/registerImageUrl', {url: imageUrl},
				{headers: {Authorization: `Bearer ${roleRes.data.token}`}}).then(res => {
				return dispatch(convertGoogleTokenSuccess(res.data, roleRes.data, imageUrl))
			}).catch(err => {
